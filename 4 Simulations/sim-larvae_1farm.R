...
Naup.fun<-function(j){
	Sum = 0;
	for (i in 0:(j-1)){
        Sum = Sum + Gn(x, (j-i)*dt, muN)*fset(tT=t_treat, fT=f_treat, r1=r1, r2=r2, tau=i*dt);
    	}#end i
	return(Sum*dt)}
...
sfExport("muN", "dt", "x", "t_treat", "r1", "r2", "D", "gamma", "n", "f_treat")
sfSource("4 Simulations/sim-functions.R")
...
t.start<-proc.time()
sfInit(parallel=TRUE, cpus=cpus)
sfExport("D","gamma","muN", "muP", "x", "Kn", "dx", "dt", "t_treat", "r1", "r2", "n", "f_treat")
sfSource("4 Simulations/sim-functions.R")
